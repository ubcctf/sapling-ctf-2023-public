...
// old function left in so the new function is clearer
...
cnode *treenode_traverse2(treenode *t, int ccase) {
  if (t == NULL) {
    return NULL;
  }
  return cnodes_join3(ccase ? cnode_create(t->c_rotated) : cnode_create(t->c),
                      treenode_traverse2(t->left, !ccase),
                      treenode_traverse2(t->right, !ccase));
}
...
/* char *flag =
 * "hello_from_the_decompiler"; */

int main(int argc, char **argv) {
  if (argc != 2) {
    printf("usage: %s <unwrapped flag>\n", argv[0]);
    exit(0);
  }

  treenode *input_t = str_to_tree(argv[1], strlen(argv[1]), 0);
  cnode *input_cns = treenode_traverse2(input_t, 0);

  puts(argv[1]);
  cnodes_print(input_cns);
